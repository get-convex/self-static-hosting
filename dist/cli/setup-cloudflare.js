 ...
 */
function hasStaticDeployment(componentName = "staticHosting") {
    try {
        const result = execSync(`npx convex run ${componentName}:getCurrentDeployment --prod`, {
            stdio: "pipe",
            encoding: "utf-8",
        });
 ...
 */
function getConvexSiteUrl(preferProd = true) {
 ...
 * Create a DNS record
 */
async function createDnsRecord(token, zoneId, record) {
    // First check if record already exists
    const existing = await cfApi(`/zones/${zoneId}/dns_records?type=${record.type}&name=${record.name}`, token);
    if (existing.success &&
        Array.isArray(existing.result) &&
        existing.result.length > 0) {
        // Update existing record
        const existingRecord = existing.result[0];
        const data = await cfApi(`/zones/${zoneId}/dns_records/${existingRecord.id}`, token, {
            method: "PUT",
 ...
 */
async function createApiToken(oauthToken, zoneName, zoneId) {
    ...
    const _accountId = accountData.result[0].id;
    // Create API token
 ...
 */
async function getSslMode(token, zoneId) {
 ...
 */
async function setSslMode(token, zoneId, mode) {
    const data = await cfApi(`/zones/${zoneId}/settings/ssl`, token, {
        method: "PATCH",
...
async function getAccountId(token) {
    ...
    if (accountData.success &&
        Array.isArray(accountData.result) &&
        accountData.result.length > 0) {
        return accountData.result[0].id;
    }
    return null;
}
/**
 ...
 */
async function deployProxyWorker(token, accountId, zoneId, domain, convexDeployment) {
...
`.trim();
    ...
    const scriptData = await cfApi(`/accounts/${accountId}/workers/scripts/${workerName}`, token, {
        method: "PUT",
        headers: {
    ...
    const existingRoutes = await cfApi(`/zones/${zoneId}/workers/routes`, token);
 ...
 */
function saveToEnv(zoneId, apiToken, domain) {
 ...
 */
function savePagesProjectToEnv(projectName) {
    const envFile = ".env.local";
    ...
    if (existsSync(envFile)) {
        ...
        if (content.includes("CLOUDFLARE_PAGES_PROJECT")) {
            // Replace existing
            const updated = content.replace(/CLOUDFLARE_PAGES_PROJECT=.*/g, newVar);
            writeFileSync(envFile, updated);
        }
        else {
 ...
 */
async function pagesProjectExists(accountId, projectName, token) {
    const data = await cfApi(`/accounts/${accountId}/pages/projects/${projectName}`, token);
    return data.success;
}
/**
 * Create a Cloudflare Pages project
 */
async function createPagesProject(accountId, projectName, token) {
 ...
 */
async function addPagesCustomDomain(accountId, projectName, domain, token) {
    const data = await cfApi(`/accounts/${accountId}/pages/projects/${projectName}/domains`, token, {
        method: "POST",
 ...
 */
async function runPagesSetup(token) {
    log("");
    log("═══════════════════════════════════════════════════════════");
    ...
    log("");
    const wantCustomDomain = await promptYesNo("Do you want to add a custom domain to your Pages project?", false);
    let customDomain = null;
    if (wantCustomDomain) {
        customDomain = await prompt("Enter your custom domain (e.g., app.example.com): ");
        if (customDomain) {
            log(`Adding custom domain: ${customDomain}...`);
            const added = await addPagesCustomDomain(accountId, projectName, customDomain, token);
            if (added) {
    ...
    log("");
    const shouldDeploy = await promptYesNo("Would you like to build and deploy static files now?");
    if (shouldDeploy) {
        log("");
        log("Building app...");
...
async function main() {
    ...
    if (!convexSiteUrl) {
        ...
        if (!manualUrl) {
            error("Convex URL is required. Run 'npx convex deploy' first to deploy to production.");
            rl.close();
            process.exit(1);
        }
    ...
    if (!selectedZone) {
        ...
        if (domainName) {
            const matchingZone = zones.find((z) => z.name.toLowerCase() === domainName.toLowerCase());
            if (matchingZone) {
    ...
    const useSubdomain = await promptYesNo(`Use a subdomain (e.g., app.${selectedZone.name})? Otherwise will use root domain (${selectedZone.name}).`, false);
    let recordName;
    let fullDomain;
    if (useSubdomain) {
    ...
    if (accountId) {
        const workerSuccess = await deployProxyWorker(activeToken, accountId, selectedZone.id, fullDomain, convexDeployment);
        if (workerSuccess) {
    ...
    if (!hasProdDeployment) {
        ...
        const shouldDeploy = await promptYesNo("Would you like to deploy your Convex backend to production now?");
        if (shouldDeploy) {
            log("");
    ...
    if (!hasStaticFiles) {
        ...
        const shouldDeployStatic = await promptYesNo("Would you like to build and deploy static files now?");
...
//# sourceMappingURL=setup-cloudflare.js.map